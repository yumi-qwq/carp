...
public final class PathingBehavior extends Behavior implements IPathingBehavior, Helper {
    ...
    public void onTick(TickEvent event) {
        ...
        if (event.getType() == TickEvent.Type.OUT) {
            // MC 26.1 port: OUT tick cancellation disabled — was unconditional cancel
            return;
        }

        expectedSegmentStart = pathStart();
        // Align with in-progress search to prevent immediate cancellation
        synchronized (pathCalcLock) {
            if (inProgress != null && !inProgress.getStart().equals(expectedSegmentStart)) {
                expectedSegmentStart = inProgress.getStart();
            }
        }
        tickPath();
        ticksElapsedSoFar++;
    ...
    private void tickPath() {
        ...
        if (pauseRequestedLastTick && safeToCancel) {
            ...
            if (unpausedLastTick) {
                ...
                // TODO: BlockBreakHelper was removed — stopBreakingBlock needs replacement
            }
        ...
        synchronized (pathPlanLock) {
            synchronized (pathCalcLock) {
                // MC 26.1 port: cancellation logic disabled to prevent interference
                // inProgress checks are handled by findPathInNewThread completion callback
            }
            if (current == null) {
                return;
            }
    ...
    public boolean cancelEverything() {
        ...
        // TODO: PathingControlManager was removed — cancelEverything needs replacement
        return doIt;
    }
    ...
    private void findPathInNewThread(final BlockPos start, final boolean talkAboutIt, CalculationContext context) {
        ...
        System.out.println("[PF] submitting A* task to executor...");
        Baritone.getExecutor().execute(() -> {
            try {
                ...
                System.out.println("[PF] result=" + calcResult.getType() + " hasPath=" + calcResult.getPath().isPresent()
                    + " segStart=" + expectedSegmentStart + " curNull=" + (current == null));
                if (current == null) {
                    if (executor.isPresent()) {
                        System.out.println("[PF] path src=" + executor.get().getPath().getSrc()
                            + " dest=" + executor.get().getPath().getDest()
                            + " len=" + executor.get().getPath().length()
                            + " containsStart=" + executor.get().getPath().positions().contains(expectedSegmentStart));
                        if (executor.get().getPath().positions().contains(expectedSegmentStart)) {
                            queuePathEvent(PathEvent.CALC_FINISHED_NOW_EXECUTING);
                            current = executor.get();
                            System.out.println("[PF] current set! pathing=" + isPathing());
                            resetEstimatedTicksToGoal(start);
                        } else {
                            System.out.println("[PF] discarding: start not in path positions");
                        }
                    } else {
                        System.out.println("[PF] no path: type=" + calcResult.getType());
                ...
                System.err.println("[PF] A* thread crashed: " + t);
                t.printStackTrace();
            }
            System.out.println("[PF] A* task finished");
        });
    }

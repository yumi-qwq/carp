...
public final class ChunkPacker {
    ...
    public static CachedChunk pack(LevelChunk chunk) {
        ...
        // @formatter:on
        return new CachedChunk(chunk.getPos().getRegionX(), chunk.getPos().getRegionZ(), height, bitSet, blocks, specialBlocks, System.currentTimeMillis());
    }
    ...
    private static PathingBlockType getPathingBlockType(BlockState state, LevelChunk chunk, int x, int y, int z) {
        ...
        if (MovementHelper.isWater(state)) {
            ...
            if (x == 0 || x == 15 || z == 0 || z == 15) {
                Vec3 flow = state.getFluidState().getFlow(chunk.getLevel(), new BlockPos(x + (chunk.getPos().getRegionX() << 4), y, z + (chunk.getPos().getRegionZ() << 4)));

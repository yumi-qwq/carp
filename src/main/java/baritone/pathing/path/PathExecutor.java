...
public class PathExecutor implements IPathExecutor, Helper {
    ...
    public boolean onTick() {
        ...
        AbstractMap.SimpleEntry<Double, BlockPos> status = closestPathPos(path);
        if (possiblyOffPath(status, MAX_DIST_FROM_PATH)) {
            ticksAway++;
            System.out.println("FAR AWAY FROM PATH FOR " + ticksAway + " TICKS. Current distance: " + status.getKey() + ". Threshold: " + MAX_DIST_FROM_PATH);
    ...
    private AbstractMap.SimpleEntry<Double, BlockPos> closestPathPos(IPath path) {
        double best = -1;
        ...
        return new AbstractMap.SimpleEntry<>(best, bestPos);
    }
    ...
    private boolean possiblyOffPath(AbstractMap.SimpleEntry<Double, BlockPos> status, double leniency) {
        double distanceFromPath = status.getKey();
    ...
    private boolean shouldSprintNextTick() {
        ...
        if (current instanceof MovementFall) {
            AbstractMap.SimpleEntry<Vec3, BlockPos> data = overrideFall((MovementFall) current);
            if (data != null) {
                BetterBlockPos fallDest = new BetterBlockPos(data.getValue());
                ...
                clearKeys();
                behavior.baritone.getLookBehavior().updateTarget(RotationUtils.calcRotationFromVec3d(ctx.playerHead(), data.getKey(), ctx.playerRotations()), false);
    ...
    private AbstractMap.SimpleEntry<Vec3, BlockPos> overrideFall(MovementFall movement) {

...
public class BaritoneProcess {
    ...
    public void onClientTick(Minecraft client) {
        ...
        if (gotoActive != wasGotoActive) {
            ...
            try {
                if (gotoActive) {
                    ...
                    diagTick = 1;
                    p.sendSystemMessage(Component.literal("[Carp] A* -> (" + x + ", " + z + ")"));
                } else {
                    baritone.getCustomGoalProcess().onLostControl();
                    baritone.getPathingBehavior().cancelEverything();
                    p.sendSystemMessage(Component.literal("[Carp] Stopped"));
                }
        ...
        if (followActive != wasFollowActive) {
            ...
            try {
                if (followActive) {
                    ...
                    p.sendSystemMessage(Component.literal("[Carp] Follow ON"));
                } else {
        ...
        if (exploreActive != wasExploreActive) {
            ...
            try {
                if (exploreActive) {
                    ...
                    p.sendSystemMessage(Component.literal("[Carp] Explore range=" + range));
                } else {
        ...
        if (mineActive != wasMineActive) {
            ...
            if (mineActive) p.sendSystemMessage(Component.literal("[Carp] Mine ON"));
        ...
        if (tunnelActive != wasTunnelActive) {
            ...
            if (tunnelActive) p.sendSystemMessage(Component.literal("[Carp] Tunnel ON"));
            else { baritone.getPathingBehavior().cancelEverything(); }
        }
        if (tunnelActive) tickTunnel(p, b);

        if (b.bsi == null && diagTick > 0 && Config.baritoneDebug.getBooleanValue()) {
            p.sendSystemMessage(Component.literal("[Carp] bsi is null!"));
        }
        ...
        try {
            ...
            if (cmd != null) {
                ...
                if (p != null && Config.baritoneDebug.getBooleanValue() && diagTick > 0 && diagTick <= 5) {
                    p.sendSystemMessage(Component.literal("[Carp] dispatch: " + cmd.commandType + " inProg=" + baritone.getPathingBehavior().getInProgress().isPresent()));
                    diagTick++;
                }
            }
    ...
    private void error(LocalPlayer p, String msg) {
        ...
        if (p != null && Config.baritoneDebug.getBooleanValue()) {
            p.sendSystemMessage(Component.literal("[Carp] " + msg));
        }
    }
    ...
    private void syncBaritoneSettings() {
        ...
        s.freeLook.value = Config.baritoneFreeLook.getBooleanValue();
